...
public final class LLMEngine: ObservableObject {
    ...
    public static let legacyPersonaKVMetadataFileName = "persona_kv_metadata.json"
    public static let neuralImprintPrefixSplitSentinel = "__NEURAL_IMPRINT_TOOLS_SPLIT_SENTINEL__"
    static let neuralImprintPrefixUserMarker = "<|im_start|>user\n"
    ...
    public struct NeuralImprintCacheStatus: Equatable, Sendable {
    ...
    public func renderNeuralImprintPrefix(
        ...
        let systemPrompt = Self.neuralImprintSystemPrompt(profileBody: profileBody)
        let messages: [ChatMessage] = [
            .system(systemPrompt),
            .user(Self.neuralImprintPrefixSplitSentinel),
        ]
        ...
        let markerTokenIDs = tokenizer.encode(
            text: Self.neuralImprintPrefixUserMarker,
            addSpecialTokens: false
        )
        guard let splitIndex = Self.neuralImprintPrefixSplitIndex(
    ...
    public func restoreNeuralImprintCache(from directory: URL) throws -> NeuralImprintCacheStatus {
        ...
        let metadataURL = Self.neuralImprintMetadataURL(directory: directory)
        let sidecar = try NeuralImprintSidecar.load(from: metadataURL)
        let artifactURL = Self.neuralImprintArtifactURL(
        ...
        let status = NeuralImprintCacheStatus(
            ...
            enableThinking: try Self.requiredNeuralImprintHeader("enable_thinking", in: artifact.metadata) == "true",
    ...
    static func neuralImprintArtifactURL(
        ...
        let defaultURLs = [
            directory.appendingPathComponent(neuralImprintArtifactFileName),
            directory.appendingPathComponent(legacyPersonaKVArtifactFileName),
        ]
    ...
    static func neuralImprintMetadataURL(directory: URL) -> URL {
        let fileManager = FileManager.default
        let candidates = [
            directory.appendingPathComponent(neuralImprintMetadataFileName),
            directory.appendingPathComponent(legacyPersonaKVMetadataFileName),
        ]
